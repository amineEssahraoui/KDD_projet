...
X_train_aug = pd.concat(
    [
        X_train,
        pd.DataFrame(noise_train, columns=noise_cols, index=X_train.index)
    ],
    axis=1
)
...
X_test_aug = pd.concat(
    [
        X_test,
        pd.DataFrame(noise_test, columns=noise_cols, index=X_test.index)
    ],
    axis=1
)
...
for model_name, model in regressors:
    r2, rmse, mae, training_time, num_trees, y_pred = evaluate(model_name, model, X_train_aug, X_test_aug, y_train, y_test)
