...
from .loss_functions import HUBERLoss, MAELoss, MSELoss, QUANTILELoss, RMSELoss
...
class LGBMRegressor(BaseEstimator):
	...
	def __init__(
		...
		self.loss = self._make_loss(loss)
	...
	def _compute_feature_importances(self) -> None:
		...
		# Map bundled importances back to original feature space.
	...
	def load_model(cls, path: str) -> "LGBMRegressor":
		...
		model = cls(
			...
			loss=state.get("loss_name", "mse"),
			huber_delta=state.get("huber_delta", 1.0),
			quantile_alpha=state.get("quantile_alpha", 0.5),
		...
		model.huber_delta = state.get("huber_delta", model.huber_delta)
		model.quantile_alpha = state.get("quantile_alpha", model.quantile_alpha)
	...
	def _make_loss(self, name: str):
		...
		if name == "rmse":
			return RMSELoss()
		if name == "huber":
			return HUBERLoss(delta=self.huber_delta)
		if name == "quantile":
			return QUANTILELoss(quantile=self.quantile_alpha)
